...
public class MyDialog extends DialogFragment {
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        ...
        if (getTag() != null) {
            ...
            } else if (getTag().equals(STUDENT_ADD_DIALOG)) {
    ...
    private Dialog getAddStudentDialog() {
        ...
        // Initialize a list to store unique roll numbers for this class
        List<String> uniqueRollNumbers = new ArrayList<>();
        ...
        add.setOnClickListener(v -> {
            ...
            if (!uniqueRollNumbers.contains(roll)) {
                // Roll number is unique, add the student and update the list
                roll_edt.setText(String.valueOf(Integer.parseInt(roll) + 1));
                name_edt.setText("");
                listener.onClick(roll, name);

                // Add the roll number to the list of unique roll numbers
                uniqueRollNumbers.add(roll);
            } else {
                // Roll number is already in use, show an error message
                Toast.makeText(getActivity(), "Roll number already exists in this class", Toast.LENGTH_SHORT).show();
            }
        });
